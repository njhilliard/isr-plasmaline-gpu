...
__global__ void
complex_mult(cufftComplex *tx, cufftComplex *echo, cufftComplex *batch, 
		     int tx_length, int n_range_gates, int range_gate_step, int range_gate_start)
{
  ...
  unsigned int i = idx / tx_length;
  unsigned int j = idx % tx_length;
  unsigned int ei = j + (i + range_gate_start) * range_gate_step;

  for (int k = 0; k < STRIDE * threads_in_block; k += threads_in_block) {
    batch[idx+k] = cuCmulf(echo[ei+k], tx[j+k]);
    __syncthreads();
  }
}
...
square_and_accumulate_sum(cufftComplex *z, float *spectrum)
{
  ...
  for (int k = 0; k < STRIDE * threads_in_block; k += threads_in_block) {
    spectrum[idx+k] += z[idx+k].x * z[idx+k].x + z[idx+k].y * z[idx+k].y;
    __syncthreads();
  }
}
		                       ...
		                       int n_range_gates, int range_gate_step, int range_gate_start)
{
  ...
  dim3 dimBlock(16, 16);
  dim3 dimGrid((n_range_gates + (dimBlock.x - 1)) / dimBlock.x / STRIDE,
  ...
  for (int i = 0 ; i < n_ipp ; i++) {
    ...
    //execution
    complex_mult<<< dimGrid, dimBlock >>>(d_z_tx, d_z_echo, d_z_batch,
